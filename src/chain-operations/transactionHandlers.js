...
import { readFairmintDataBySecurityId, readStakeholderById } from "../db/operations/read.js";
import {
...
export const handleStockIssuance = async (stock, issuerId, timestamp) => {
    ...
    const createdStockIssuance = await upsertStockIssuanceById(_id, {
        ...
        date: dateToUse, // priortise historical date if available
        custom_id,
...
export const handleConvertibleIssuance = async (convertible, issuerId, timestamp, hash) => {
    ...
    console.log(`✅ | IssuerAuthorizedSharesAdjusted confirmation onchain with date ${new Date(Date.now()).toLocaleDateString("en-US", options)}`);
};
...
export const handleStockConsolidation = async (data, issuerId, timestamp) => {
    ...
    result = await handleStockConsolidation.create({
        _id,
